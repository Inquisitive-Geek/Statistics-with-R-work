...
ames_test_prediction_aic <- exp(predict(model.AIC, ames_test))
mean_predicted_price <- mean(ames_test_prediction_aic)
```
...
resid.full.aic <- ames_test$price - ames_test_prediction_aic

# Calculate RMSE
rmse.aic <- sqrt(mean(resid.full.aic^2))
rmse.aic

# Calculate RMSE of Full Model and AIC
ames_train_prediction_full <- exp(predict(model.full))

resid.full.train <- ames_train$price - ames_train_prediction_full

rmse.full.train <- sqrt(mean(resid.full.train^2))
rmse.full.train

ames_train_prediction_aic <- exp(predict(model.AIC))

resid.AIC.train <- ames_train$price - ames_train_prediction_aic

rmse.AIC.train <- sqrt(mean(resid.AIC.train^2))
rmse.AIC.train
# model.AIC.summary <- summary(model.AIC)
```
...
coverage.prob.BIC <- mean(ames_test$price > predict.BIC.test[,"lwr"] &
...
# Get coverage
pred.test.MPM.coverage <- out %>% summarise(cover = sum(price >= lwr & price <= upr)/n())
